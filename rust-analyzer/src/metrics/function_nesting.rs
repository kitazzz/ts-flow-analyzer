    ...
    match stmt {
        ...
        Statement::IfStatement(s) => {
            count_local_functions_stmt(&s.consequent)
                + s.alternate
                    .as_ref()
                    .map_or(0, |a| count_local_functions_stmt(a))
        }
        ...
        Statement::SwitchStatement(sw) => sw
            .cases
            .iter()
            .map(|c| count_local_functions_stmts(&c.consequent))
            .sum(),
        Statement::TryStatement(t) => {
            count_local_functions_stmts(&t.block.body)
                + t.handler
                    .as_ref()
                    .map_or(0, |h| count_local_functions_stmts(&h.body.body))
                + t.finalizer
                    .as_ref()
                    .map_or(0, |f| count_local_functions_stmts(&f.body))
        }
        Statement::ReturnStatement(ret) => ret
            .argument
            .as_ref()
            .map_or(0, |a| count_local_functions_expr(a)),
        ...
        Statement::VariableDeclaration(var_decl) => var_decl
            .declarations
            .iter()
            .map(|d| d.init.as_ref().map_or(0, |i| count_local_functions_expr(i)))
            .sum(),
        _ => 0,
    }
}
            ...
            for arg in &call.arguments {
                match arg {
                    Argument::SpreadElement(spread) => {
                        count += count_local_functions_expr(&spread.argument)
                    }
    ...
    match stmt {
        ...
        Statement::IfStatement(s) => {
            ...
            let d2 = s
                .alternate
                .as_ref()
                .map_or(depth, |a| walk_fn_depth_stmt(a, depth, false));
            d1.max(d2)
        }
        ...
        Statement::SwitchStatement(sw) => sw
            .cases
            .iter()
            .map(|c| walk_fn_depth_stmts(&c.consequent, depth, false))
            .max()
            .unwrap_or(depth),
        Statement::TryStatement(t) => {
            ...
            let d2 = t
                .handler
                .as_ref()
                .map_or(depth, |h| walk_fn_depth_stmts(&h.body.body, depth, false));
            let d3 = t
                .finalizer
                .as_ref()
                .map_or(depth, |f| walk_fn_depth_stmts(&f.body, depth, false));
            d1.max(d2).max(d3)
        }
        Statement::ReturnStatement(ret) => ret
            .argument
            .as_ref()
            .map_or(depth, |a| walk_fn_depth_expr(a, depth, false)),
        Statement::ExpressionStatement(expr_stmt) => {
        ...
        Statement::VariableDeclaration(var_decl) => var_decl
            ...
            .map(|d| {
                d.init
                    .as_ref()
                    .map_or(depth, |i| walk_fn_depth_expr(i, depth, false))
            })
            .max()
            .unwrap_or(depth),
        _ => depth,
    ...
    match expr {
        ...
        Expression::CallExpression(call) => {
            ...
            for arg in &call.arguments {
                let d = match arg {
                    Argument::SpreadElement(spread) => {
                        walk_fn_depth_expr(&spread.argument, depth, false)
                    }
        ...
        Expression::LogicalExpression(log) => walk_fn_depth_expr(&log.left, depth, false)
            .max(walk_fn_depth_expr(&log.right, depth, false)),
        Expression::ConditionalExpression(cond) => walk_fn_depth_expr(&cond.test, depth, false)
        ...
        Expression::TSNonNullExpression(inner) => {
            walk_fn_depth_expr(&inner.expression, depth, false)
        }
    ...
    match stmt {
        ...
        Statement::IfStatement(s) => {
            ...
            let d2 = s
                .alternate
                .as_ref()
                .map_or(depth, |a| calc_callback_nesting_depth_stmt(a, depth));
            d1.max(d2)
        }
        ...
        Statement::SwitchStatement(sw) => sw
            .cases
            .iter()
            .map(|c| calc_callback_nesting_depth_stmts(&c.consequent, depth))
            .max()
            .unwrap_or(depth),
        Statement::TryStatement(t) => {
            let d1 = calc_callback_nesting_depth_stmts(&t.block.body, depth);
            let d2 = t.handler.as_ref().map_or(depth, |h| {
                calc_callback_nesting_depth_stmts(&h.body.body, depth)
            });
            let d3 = t
                .finalizer
                .as_ref()
                .map_or(depth, |f| calc_callback_nesting_depth_stmts(&f.body, depth));
            d1.max(d2).max(d3)
        }
        Statement::ReturnStatement(ret) => ret
            .argument
            .as_ref()
            .map_or(depth, |a| calc_callback_nesting_expr(a, depth)),
        Statement::ExpressionStatement(expr_stmt) => {
        ...
        Statement::VariableDeclaration(var_decl) => var_decl
            ...
            .map(|d| {
                d.init
                    .as_ref()
                    .map_or(depth, |i| calc_callback_nesting_expr(i, depth))
            })
            .max()
            .unwrap_or(depth),
...
fn calc_callback_nesting_expr(expr: &Expression<'_>, depth: u32) -> u32 {
    match expr {
        ...
        Expression::LogicalExpression(log) => calc_callback_nesting_expr(&log.left, depth)
            .max(calc_callback_nesting_expr(&log.right, depth)),
        Expression::ConditionalExpression(cond) => calc_callback_nesting_expr(&cond.test, depth)
        ...
        Expression::TSNonNullExpression(inner) => {
            calc_callback_nesting_expr(&inner.expression, depth)
        }
